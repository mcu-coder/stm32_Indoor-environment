...
void Gpio_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  = RELAY_JR_PIN; 							//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 					//设置成推挽输出
 	GPIO_Init(RELAY_JR_PORT, &GPIO_InitStructure);						//根据设定参数初始化
	GPIO_ResetBits(RELAY_JR_PORT,RELAY_JR_PIN);							  //IO口输出低
	
	
	GPIO_InitStructure.GPIO_Pin  = RELAY_ZL_PIN; 							//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 					//设置成推挽输出
 	GPIO_Init(RELAY_ZL_PORT, &GPIO_InitStructure);						//根据设定参数初始化
	GPIO_ResetBits(RELAY_ZL_PORT,RELAY_ZL_PIN);							  //IO口输出低
	
	GPIO_InitStructure.GPIO_Pin  = MOTOR_IN0_PIN; 						//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 					//设置成推挽输出
	GPIO_Init(MOTOR_IN0_PORT, &GPIO_InitStructure);						//根据设定参数初始化
	GPIO_ResetBits(MOTOR_IN0_PORT,MOTOR_IN0_PIN); 				 		//IO口输出低
	...
	GPIO_ResetBits(MOTOR_IN1_PORT,MOTOR_IN1_PIN); 				 		//IO口输出低
		...
		GPIO_InitStructure.GPIO_Pin  = RELAY_ZM_PIN; 							//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 					//设置成推挽输出
 	GPIO_Init(RELAY_ZM_PORT, &GPIO_InitStructure);						//根据设定参数初始化
	GPIO_ResetBits(RELAY_ZM_PORT,RELAY_ZM_PIN);							  //IO口输出低
}
